...
modalParam, popupParam, sliderParam, spinProgressParam, select2Param) {
    ...
    $(document).ready(function(){
        ...
        $('#btn_new_personal_objective').click(function() {
          ...
          let create_objective_promise = new personalObjectiveModelParam.newPersonalObjective(personal_objective, team_key_result_id);
          ...
          keypress: function(e){
            ...
            if(key == 13){
              ...
              if(updated_objective != original_objective){
                let edit_objective_promise = new personalObjectiveModelParam.editPersonalObjective(
          ...
          keypress: function(e){
            ...
            if(key == 13){
              if($(this).find('.form-new-key-result').val() != '') {
                ...
                let create_personal_kr_promise = new personalKeyResultModelParam.newPersonalKeyResult(
                    temp_personal_key_result, id_substring
                );   
                create_personal_kr_promise.then(function(value) {
                  $('#notification_message').text(value);
                  $('#notification_modal').modal('show');
                }, function(reason) {
                  $('#notification_message').text(reason);
        ...
        $('#btn_update_progress').click(function() {
          ...
          update_kr_progress_promise.then(function(value) {
            $('#notification_message').text(value);
            $('#notification_modal').modal('show');
          }, function(reason) {
            $('#notification_message').text(reason);
        ...
        $('input[type=checkbox]').click(function() {
          ...
          {
            let update_kr_status_promise = new personalKeyResultModelParam.updatePersonalKeyResultStatus(key_result_id, false);
            update_kr_status_promise.then(function(value) {
              $('#notification_message').text(value);
              $('#notification_modal').modal('show'); 
            }, function(reason) {
              $('#notification_message').text(reason);
          ...
          {
            let update_kr_status_promise = new personalKeyResultModelParam.updatePersonalKeyResultStatus(key_result_id, true);
            update_kr_status_promise.then(function(value) {
              $('#notification_message').text(value);
              $('#notification_modal').modal('show'); 
            }, function(reason) {
          ...
          keypress: function(e) {
            ...
            if(key == 13){
              ...
              if(updated_key_result != original_key_result && updated_key_result != ''){
                let edit_kr_promise = new personalKeyResultModelParam.editPersonalKeyResult(
                    editing_key_result_id, updated_key_result, original_key_result
                );
                edit_kr_promise.then(function(value) {
                  $('#notification_message').text(value);
                  $('#notification_modal').modal('show');
                }, function(reason) {
                  $('#notification_message').text(reason);
